...
local on_attach = function(client, bufnr)
	...
	keymap.set("n", "<leader>rn", "<cmd>lua vim.lsp.buf.rename()<CR>") -- mass rename

	if client.name == "pyright" then
		keymap.set("n", "<leader>sf", "<cmd>lua vim.lsp.buf.format()<CR>") -- format python file using black
		keymap.set("n", "<leader>oi", "<cmd>%!isort --profile black - 2>/dev/null<CR>") -- organize imports
	end
	-- typescript specific keymaps (e.g. rename file and update imports)
	if client.name == "tsserver" then
		keymap.set("n", "<leader>rf", "<cmd>TypescriptRenameFile<CR>") -- rename file and update imports
		keymap.set("n", "<leader>oi", "<cmd>TypescriptOrganizeImports<CR>") -- organize imports
